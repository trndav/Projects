...
#             # Only display buttons for player-controlled bubbles
#             if self.is_player:
#                 if self.grow_button:
#                     pygame.draw.rect(screen, WHITE, self.grow_button)
#                     pygame.draw.rect(screen, BLACK, self.grow_button, 2)
...
#     def fight(self, other_bubble):
#         if self.color == other_bubble.color:
#             # Transfer population between same-color bubbles
#             transfer_rate = TRANSFER_RATE_SLOW if self.is_neutral or other_bubble.is_neutral else TRANSFER_RATE_NORMAL
#             if self.population > other_bubble.population:
#                 transfer_amount = (self.population - other_bubble.population) * transfer_rate / FPS
#                 self.population -= transfer_amount
...
#                 self.population += transfer_amount
#         else:
#             # Bubbles fight if they are different colors
#             if self.population > 0 and other_bubble.population > 0:
#                 self.population -= other_bubble.attack_rate / FPS
#                 other_bubble.population -= self.attack_rate / FPS
...
#             # Any bubble can be conquered when population reaches zero
#             if self.population <= 0:
#                 self.become_owned(other_bubble)
#             if other_bubble.population <= 0:
#                 other_bubble.become_owned(self)
...
#         # Inherit growth rate and growth level from the owner
#         self.growth_rate = owner_bubble.growth_rate
#         self.growth_level = owner_bubble.growth_level
#         self.growth_paused = owner_bubble.growth_paused

#         # Update is_player attribute
#         self.is_player = owner_bubble.is_player

#         # Assign buttons if the bubble now belongs to the player
#         if self.is_player:
#             self.grow_button = pygame.Rect(WIDTH // 2 - 70, GAME_AREA_HEIGHT + 10, 60, 30)
#             self.attack_button = pygame.Rect(WIDTH // 2 + 10, GAME_AREA_HEIGHT + 10, 60, 30)
#         else:
...
#         # Remove existing connections involving this bubble
#         connections_to_remove = [conn for conn in connections if self in conn]
#         for conn in connections_to_remove:
#             connections.remove(conn)
...
#     # Collect all bubbles
#     all_bubbles = [player_bubble, enemy_bubble] + neutral_bubbles

#     # Update game-over logic
#     player_bubbles_remaining = any(bubble.color == player_bubble.color and bubble.population > 0 for bubble in all_bubbles)
#     enemy_bubbles_remaining = any(bubble.color == enemy_bubble.color and bubble.population > 0 for bubble in all_bubbles)

#     if not player_bubbles_remaining:
#         game_over = True
#         winner = enemy_bubble
#     elif not enemy_bubbles_remaining:
#         game_over = True
#         winner = player_bubble
...
while running:
    ...
    if not enemy_bubbles_remaining:
        game_over = True
        winner = player_bubble
    elif not player_bubbles_remaining:
        game_over = True
        winner = enemy_bubble
    ...
    if player_bubble.population <= 0 or enemy_bubble.population <= 0:
        if player_bubble.population <= 0:
            ...
            if not enemy_bubbles_remaining:  # If no red neutral bubbles are left
                game_over = True
                winner = player_bubble
                enemy_bubble.population = 0  # Hide the defeated bubble
                enemy_bubble.growth_paused = True
